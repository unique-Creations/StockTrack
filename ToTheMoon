...
# ts = TimeSeries(key='ZTQCL7FXGN9MMH9B', output_format='pandas')
np.random.seed(314)
...
SCALE = True
scare_str = f"sc-{int(SCALE)}"

# Whether to shuffle the dataset
SHUFFLE = True
shuffle_str = f"sbd-{int(SHUFFLE)}"
...
# Amazon Stock for TESTING
ticker = "AMZN"
ticker_data_filename = os.path.join("data", f"{ticker}_{date_now}.csv")

# model name to save, making it as unique as possible based on parameters
model_name = f"{date_now}_{ticker}-{shuffle_str}-{scare_str}-{split_by_date_str}-{LOSS}-{OPTIMIZER}-{CELL.__name__}-" \
             f"seq-{N_STEPS}-step-{LOOKUP_STEP}-layers-{N_LAYERS}-units-{UNITS}"
if BIDIRECTIONAL:
    model_name += "-b"
...
def load_data(ticker, n_steps=50, scale=True,
    ...
    # last 'lookup_step' colummns contains NaN in future column
    ...
    # for instance, if n_steps=50 and lookup_step=10, last_sequence should be of 60 length
    ...
    if split_by_date:
        ...
        result["y_test"] = X[train_samples:]

        if shuffle:
            # shuffle the datasets for training (if shuffle parameter is set)
            shuffle_in_unison(result["X_train"], result["y_train"])
            shuffle_in_unison(result["X_train"], result["y_test"])
...
def create_model(sequence_length, n_features, units=256, cell=LSTM, n_layers=2, dropout=0, loss="mean_absolute_error",
...
if __name__ == '__main__':
    # load the data
    data = load_data(ticker, N_STEPS, scale=SCALE, split_by_date=SPLIT_BY_DATE, shuffle=SHUFFLE,
    ...
    #                     validation_data=(data["X_test"], data["y_test"]), callbacks=[checkpointer, tensorboard],
    #                     verbose=1)
